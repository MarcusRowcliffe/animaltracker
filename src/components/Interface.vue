<template>
    <div width=100% id="interface">
    	<svg id="main" width="100%" height="540" overflow="hidden">
            <g>
        ...
        <div id="controls" class="container">
            <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                ...
                <button class="btn" @click="resetView">Reset view [r]</button>
                <button class="btn" @click="switchMode">{{ mode == 'add' ? 'Add mode' : 'Edit mode' }} [m]</button>
                <button class="btn" @click="newSequence">Add sequence [a]</button>
                <button class="btn" @click="deletePoint" :disabled="current.sequence < 0">Delete point [d]</button>
            </div>
        </div>
    </div>
</template>
    ...
    methods: {
        addPoint (event) {
            ...
            if (this.current.sequence >= 0) {
                ...
                this.sequences.find(seq => seq.id == this.current.sequence).points.push(
                    {x: x, y: y, frame: this.current.frame,
                        img_name: this.images[this.current.frame].name,
                        lmdate: this.images[this.current.frame].lmdate}
                )
            }
        },
        deletePoint () {
