<template>
	<div id="app">
        <div id="header" width="100%" class="container">
            <div class="btn-toolbar" width="960" role="toolbar" aria-label="Toolbar with button groups">
                <input type="file" style="float: left" class="btn" @change="handleFiles" accept="image/*" multiple>
                <button type="button" class="btn btn-secondary" @click="exportCSV">Export CSV</button>
                <div class="input-group float-right ml-auto" id="options">
...
export default {
    ...
    methods: {
        exportCSV () {
		    ...
		    var output = [["x", "y", "frame_number", "sequence_id", "sequence_annotation", "number_of_points", "filename", "file_last_mod_date"]];
            var sep = ',';
            for (var i = 0; i < this.sequences.length; i++) {
                ...
                for (var j = 0; j < points.length; j++) {
                    ...
                    output.push([points[j].x, points[j].y, points[j].frame, i, this.sequences[i].name, points.length, points[j].img_name, points[j].lmdate].join(','));
                }
            }
            output = output.join('\n');
        ...
        getImage (file, progress) {
            ...
            .then(function (result) {
                return new Promise(function (resolve, reject) {
                     ...
                     image_element.name = file.name;
                     image_element.lmdate = file.lastModifiedDate;
                     progress.loaded++;
                })
